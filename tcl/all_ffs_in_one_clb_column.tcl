...
# Example:
#
#   vivado -mode batch -source all_ffs_in_one_clb_column.tcl -notrace -tclargs xcu250-figd2104-2L-e CLEM output.bit
#
...
proc main { fpga_part tile_type bitstream_out_name } {
  ...
  create_project "all_ffs_in_one_clb_column" -in_memory -part ${fpga_part}
  ...
  set reg_pattern "SLICE_X(\\d+)Y(\\d+)/(\[ABCDEFGH\]FF2?)"
  ...
  set regs [lsort -dictionary [get_bels -of_objects ${tiles} -regexp ${reg_pattern}]]
  # puts ${regs}

  set g_size [llength ${regs}]
  puts "Creating FF array of size ${g_size}"
  ...
  # synthesis entirely.
  read_verilog $::script_path/ff_array.v
  synth_design -top ff_array -no_timing_driven -generic G_SIZE=${g_size}
  ...
  foreach idx [struct::list iota ${g_size}] ff_loc ${regs} {
    # The name we query here is the name used in the verilog file.
    set ff_cell [get_cells "ff_gen[${idx}].ff_inst"]
